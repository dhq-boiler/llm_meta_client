...
export default class extends Controller {
  ...
  #renderServerList() {
    ...
    for (const server of this.mcpServers) {
      ...
      serverDiv.innerHTML = `
        <div class="mcp-server-header" data-action="click->tool-selector#toggleServer" data-server-uuid="${server.uuid}">
        ...
        <div class="mcp-server-tools" data-server-tools="${server.uuid}" style="display: none;" data-loaded="${server.tools && server.tools.length > 0 ? "true" : "false"}">
  ...
  async #fetchToolsForServer(serverUuid, container) {
    ...
    try {
      const response = await fetch(
        `/api/mcp_servers/${serverUuid}/tools`,
        {
          headers: { Accept: "application/json" },
        }
      )
